...
class BplusTree:
    ...
    def insert(self, value, key):
        value = str(value)
        self.root.messages.append(value)
    ...
    def flush(self, node: Node):
        ...
        for msg in node.messages:
            ...
            if (len(node.nextKey[loc].messages) == len(node.nextKey[loc].buffer)):
                #if full, then flush this child first: (recursively)
                self.flush(node.nextKey[loc])
            else:
...
bplustree.insert('40', {"Name":"Ifrah Ilyas"})
...
if(bplustree.find('40', {"Name":"Ifrah Ilyas"})):
    print("Found")
else:
    print("Not found")